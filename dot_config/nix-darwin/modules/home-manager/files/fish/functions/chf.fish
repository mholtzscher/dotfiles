...
echo "The following files will be forgotten in chezmoi:"
for file in $selected_files
    echo "  $file"
end

read -P "Proceed with 'chezmoi forget'? (Y/n) " -l confirmation

if not string match -q -- n (string lower -- "$confirmation")
    # Add the selected files to chezmoi
    echo "Forgetting files in chezmoi..."
    chezmoi forget $selected_files
